...
description: 为 Vibe Coding 项目建立可追溯、经确认的协作契约。用于软件仓库的新建、修改、规划、排障、文档、测试、打包和交付；也用于执行 `init`，根据映射后的 PRD/PDD 生成并维护关联需求记录的项目总进度台账。
---
...
1. 先完成本节的契约审视，读取 `DOCUMENT_MAP.md` 映射后的完整 PRD、PDD、需求台账、决策台账和现有进度文档。PRD/PDD 名称与职责不一致时，以地图和文档内容为准，在总进度中写明实际来源。
2. 沿用现有项目总进度文档；缺失时，从 `assets/governance-starter/docs/PROGRESS.md.template` 创建一个。更新 `DOCUMENT_MAP.md`，使项目总进度、需求台账和功能进度文档可定位。
3. 按 PRD 的功能域和 PDD 的技术域建立两张交付台账。每一行至少写明：来源章节、当前状态、关联 REQ、完成条件、依赖或验证证据、下一步。
4. 单独列出已验证 POC。POC 只能标为“已通过（POC）”，不得写成“已完成（V1）”；没有代码或验证证据的 V1 项必须标为“待开发”或“开发中”。
5. 为启动中的 V1 或其他大任务建立 REQ 和功能进度记录。后续每次功能推进必须从关联 REQ 开始，同步更新该 REQ、功能进度和项目总进度的对应行；不能只更新总进度中的阶段概述。
6. 对照 PRD/PDD 的全部适用一级章节检查覆盖：产品范围、用户流程、页面、数据与权限、技术实现、验证与发布。明确标注不纳入本版本的内容和理由。
